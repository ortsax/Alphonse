...
var (
	...
	anticallMode    = "warn" // "warn" or "block" — default is warn
	anticallCodes   []string // calling codes; empty = apply to all callers

	// anticallWarnedMu guards anticallWarned.
	// A phone number in this map has already received the first-warning DM;
	// the next call from that number triggers an actual reject+block.
	anticallWarnedMu sync.Mutex
	anticallWarned   = map[string]bool{}
)

// whatsappCallingCodes maps ITU-T calling codes to country names
// for the 180 countries supported by WhatsApp.
...
// CallOfferHook is called when an incoming call offer is received.
type CallOfferHook func(client *whatsmeow.Client, evt *events.CallOffer)

var callOfferHooks []CallOfferHook

// RegisterCallOfferHook registers fn to run on every incoming call offer.
...
// anticallHook handles incoming call offer events.
// The call is ALWAYS rejected. Mode only controls the follow-up action.
func anticallHook(client *whatsmeow.Client, evt *events.CallOffer) {
	...
	// If country codes are configured, only act on calls from those codes.
	if len(codes) > 0 {
	...
	client.RejectCall(context.Background(), evt.From, evt.CallID) //nolint:errcheck
	...
	case "block":
		anticallWarnedMu.Lock()
		alreadyWarned := anticallWarned[callerPhone]
		if !alreadyWarned {
			// First call: warn that next call triggers a block.
			anticallWarned[callerPhone] = true
			anticallWarnedMu.Unlock()
			dmToUser(mentionText(T().AnticallCallerBlockWarn))
		} else {
			// Second call: block + notify.
			delete(anticallWarned, callerPhone)
			anticallWarnedMu.Unlock()
			// TODO: blocking is pending a fix for LID-based accounts
			dmToUser(mentionText(T().AnticallCallerBlocked))
		}
	}
}

// matchCallingCode extracts the ITU calling code from a phone number string
// (international format, no leading +). Tries 3-digit, 2-digit, 1-digit prefixes.
