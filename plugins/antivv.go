...
// antivvOn is a global toggle — 1 = on, 0 = off.
var antivvOn int32
...
func init() {
	...
	Register(&Command{
		...
		Func: func(ctx *Context) error {
			...
			fwd := revealViewOnce(quoted, ci.GetStanzaID(), ci.GetParticipant(), ctx.Event.Info.Chat.String())
			if fwd == nil {
				ctx.Reply("The quoted message is not a view-once media.")
				return nil
			}

			ctx.Client.SendMessage(context.Background(), ctx.Event.Info.Chat, fwd)
			return nil
		},
	})
...
// revealViewOnce strips viewOnce from a media message and attaches a quote context.
// Returns nil if the message is not a view-once image/video/audio.
...
// vvQuoteContext builds a ContextInfo that quotes the original view-once message.
func vvQuoteContext(evt *events.Message) *waProto.ContextInfo {
